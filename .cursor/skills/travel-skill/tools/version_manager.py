...
def _resolve_paths(paths: Optional[List[str]] = None) -> List[str]:
...
def create_snapshot(note: str = "", paths: Optional[List[str]] = None) -> SnapshotMeta:
    version = _next_version_id()
    version_dir = snapshot_dir(version)
    files = _resolve_paths(paths)

    for rel_path in files:
        source = SKILL_ROOT / rel_path
...
def rollback_to(version_id: str, paths: Optional[List[str]] = None) -> SnapshotMeta:
    versions = load_versions()
