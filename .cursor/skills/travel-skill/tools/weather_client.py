...
def get_current_weather(city: str) -> ApiResult:
    ...
    query = urllib.parse.urlencode({"location": city_code, "key": WEATHER_KEY})
    url = f"{WEATHER_BASE_URL}/weather/now?{query}"
    ...
    try:
        ...
        current = WeatherCurrent(
            ...
            wind_power=now.get("windScale", ""),
            report_time=payload.get("updateTime", ""),
        )
...
def get_forecast(city: str, days: int = 3) -> ApiResult:
    ...
    city_code = resolve_city_code(city)
    query = urllib.parse.urlencode({"location": city_code, "key": WEATHER_KEY})
    url = f"{WEATHER_BASE_URL}/weather/3d?{query}"
    ...
    try:
        ...
        forecast = [
            WeatherForecastItem(
                ...
                day_temp=item.get("tempMax", ""),
                night_temp=item.get("tempMin", ""),
                day_wind=item.get("windDirDay", ""),
                day_power=item.get("windScaleDay", ""),
            )
            for item in daily
        ]
...
def build_weather_summary(city: str) -> ApiResult:
    ...
    current: WeatherCurrent = current_result.data
    forecast: List[WeatherForecastItem] = forecast_result.data
    summary = WeatherResult(
        city=city,
        current=current,
        forecast=forecast,
        travel_advice=_build_advice(current, forecast),
    )
