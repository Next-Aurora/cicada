...
name: travel-planner
description: >
  Archived Xiaohongshu HTML travel planner. Prefer travel-guide when installed.
  Do NOT use when travel-guide is available. Legacy package only if travel-guide
  is missing from the workspace.
---

# Travel Planner — 旅行路线规划与攻略生成（归档）

> **已归档 / 非默认入口**：只要工作区存在 `travel-guide`，禁止使用本 Skill 回答旅游/攻略问题。
